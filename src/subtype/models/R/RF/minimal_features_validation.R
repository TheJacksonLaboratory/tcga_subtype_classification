...
reduced.model <- randomForest(tumor.type  ~ . , data = train.dat ,ntree= 100)
...
features.common = intersect(features,gse.p18.anns[,11])
features.common = intersect(features.common,gse.p21.anns[,11])
features.common = intersect(features.common,pdx.anns$gene_name)
...
gse.p21.anns.sel <- gse.p21.anns[gse.p21.anns[,11] %in% features.common,]
...
gse.21.uniq <- gse.p21.anns.ord[!duplicated(gse.p21.anns.ord[,11]),]
gse.21.exp.uniq <- gse.21.ord[rownames(gse.21.uniq),]
rownames(gse.21.exp.uniq) <- gse.21.uniq[,11]

gse.p18.anns.sel <- gse.p18.anns[gse.p18.anns[,11] %in% features.common,]
...
gse.18.uniq <- gse.p18.anns.ord[!duplicated(gse.p18.anns.ord[,11]),]
gse.18.exp.uniq <- gse.18.ord[rownames(gse.18.uniq),]
rownames(gse.18.exp.uniq) <- gse.18.uniq[,11]
...
pdx.anns.sel <- pdx.anns[pdx.anns$gene_name %in% features.common,]
pdx.exp <- Biobase::exprs(pdx.eset[rownames(pdx.anns.sel),])
r.mu <- apply(pdx.exp,1,mean)
o <- order(r.mu,decreasing=T)
pdx.ord <- pdx.exp[o,]
pdx.anns.ord <- pdx.anns.sel[o,]
pdx.uniq <- pdx.anns.ord[!duplicated(pdx.anns.ord$gene_name),]
pdx.exp.uniq <- pdx.ord[rownames(pdx.uniq),]
rownames(pdx.exp.uniq) <- pdx.uniq$gene_name
...
save(pdx.cm, gse18549.cm, gse2109.cm, file ="confusion_matrices_reduced_features.rdata")
