...
for(i in 1:K)
{
	...
	train.exp <- train[,-c(t.cl.idx)]
	train.cl <- train[,t.cl.idx]
	knn.pred <- knn(train = train.exp, test = test , cl = train.cl, k=  10)
        KNN.predicted.label[test.idx] <- as.character(knn.pred)
}

KNN.predicted.label = factor(KNN.predicted.label)
KNN.res <- confusionMatrix(KNN.predicted.label,true.label)

write.csv(KNN.res[[2]],"contingency_table_SVM.csv")
write.csv(KNN.res[[4]][,1:7],caption= "performance_metrics_DLDA.csv")
